...
export async function createAppointmentControlller(req:Request, res: Response, next:NextFunction): Promise<void> {
    try {
...
export async function listAppointmentsController(req:Request,res:Response, next: NextFunction): Promise<void> {
    try {
        ...
        const filters :AppointmentFilterDTO = {
            ...
            dentistId: req.query.dentinstId as string,
...
export async function uptadeAppointmentStatusCotroller(req:Request, res: Response, next: NextFunction): Promise<void>{
    try{
        const {tenantId,clinicId} = req.user!
        const {id}= req.params
        const appointment = await appointmentService.uptadeAppointmentStatus(tenantId,clinicId,id as string, req.body as UpdateAppointStatusDTO)
        res.status(200).json(appointment)
...
export async function deleteAppointmentController(req:Request,res:Response,next:NextFunction):Promise<void> {
    try {
        const {tenantId,clinicId} = req.user!
        const {id} = req.params
        const appointment = await appointmentService.deleteAppointment(tenantId,clinicId,id as string)
        res.status(204).send()
    } catch (error) {
        next(error)
    }
